...
export function Header() {
  ...
  useEffect(() => {
    ...
    const handleBidBalanceUpdate = (data: {
      userId: string;
      newBalance: number;
    }) => {
                ...
                <>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="ghost"
                        className="bg-gray-700 hover:bg-gray-600 border border-gray-600 hover:border-blue-400 rounded-xl h-11 px-4 transition-all"
                    ...
                    >
                      <DropdownMenuItem asChild>
                        <Link
                          href="/profile"
                          className="flex items-center cursor-pointer text-gray-300 hover:text-white hover:bg-gray-700"
                        ...
                        <div className="text-center p-6 bg-gray-700 rounded-xl border border-gray-600">
                          ...
                          <p className="text-sm text-gray-300 mb-4">
                            {settings?.language === 'ru' &&
                              'Войдите, чтобы начать участвовать в аукционах'}
                            {settings?.language === 'en' &&
                              'Login to start participating in auctions'}
                            {settings?.language === 'ka' &&
                              'შედით აუქციონებში მონაწილეობისთვის'}
                            {!settings?.language &&
                              'შედით აუქციონებში მონაწილეობისთვის'}
                          </p>
                          <Button
                  ...
                  <div className="flex-1 p-6">
                    ...
                    {isAuthenticated && user?.role === 'admin' && (
                      <>
                        <Separator className="my-4 bg-gray-700" />
                        <div className="space-y-2">
                          <h4 className="text-sm font-semibold text-gray-400 uppercase tracking-wide px-3">
                            {t('admin')}
                          </h4>
