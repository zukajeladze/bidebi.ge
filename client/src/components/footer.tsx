          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center text-gray-300 text-sm">
                ...
                <span>{settings?.contactEmail || 'info@bidebi.ge.ge'}</span>
              </div>
              <div className="flex items-center text-gray-300 text-sm">
        ...
        <div className="border-t border-gray-700 mt-8 pt-6">
          <div className="flex flex-col md:flex-row items-center justify-between">
            <p className="text-gray-400 text-sm mb-2 md:mb-0">
              © {new Date().getFullYear()} {settings?.siteName || 'BIDEBI.GE'}.
              {settings?.language === 'en' && ' All rights reserved.'}
              {settings?.language === 'ka' && ' ყველა უფლება დაცულია.'}
            </p>
            <p className="text-blue-400 text-sm">
              {settings?.language === 'ru' &&
                'Первая пенни-аукционная платформа в России'}
              {settings?.language === 'en' &&
                'First penny auction platform in Russia'}
              {settings?.language === 'ka' &&
                'პირველი პენი აუქციონის პლატფორმა საქართველოში'}
              {!settings?.language &&
                'Первая пенни-аукционная платформа в России'}
            </p>
          </div>
